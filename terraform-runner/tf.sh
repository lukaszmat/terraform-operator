...
if [[ $(ls $vardir | wc -l) -gt 0 ]]; then
  vars="-var-file $(ls $vardir | sort -n | join_by ' -var-file ')"
fi
...
case "$TFO_RUNNER" in
    init | init-delete)
        ...
        ;;
    plan)
        terraform plan -var-file tfvars $vars -out tfplan $module 2>&1 | tee "$out"/"$TFO_RUNNER".out
        ;;
    plan-delete)
        terraform plan -var-file tfvars $vars -destroy -out tfplan $module 2>&1 | tee "$out"/"$TFO_RUNNER".out
        ;;
    apply | apply-delete)
